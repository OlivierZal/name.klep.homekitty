...
module.exports.MappedDevice = class MappedDevice {
  ...
  accessorize() {
    ...
    this.#maps.sort((x, y) => x.name === 'thermostat' ? -1 : y.name === 'thermostat' ? 1 : 0);
            ...
            const characteristics = [ characteristicMap.characteristics ].flat().map(klass => {
              ...
              if (map.onUpdate) {
                characteristic.on('change', async ({ oldValue, newValue }) => {
                  ...
                  map.onUpdate({ accessory, characteristic : characteristic.constructor.name, oldValue, newValue, service, device, capability });
                });
              }
